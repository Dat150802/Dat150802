...
const syncJsonFields=document.getElementById('sync-jsonstorage-fields');
const syncGithubFields=document.getElementById('sync-github-fields');
const syncProviderField=syncForm?syncForm.elements.provider:null;
...
window.addEventListener('klc:sync-endpoint-updated',evt=>{
  const endpoint=(evt?.detail?.endpoint||'').trim();
  if(syncEndpointDisplay){
    syncEndpointDisplay.value=endpoint;
  }
  if(syncForm?.elements?.endpoint){
    syncForm.elements.endpoint.value=endpoint;
  }
  if(syncForm?.elements?.enabled && endpoint){
    syncForm.elements.enabled.checked=true;
    toggleSyncInputs(true);
  }
  if(endpoint && evt?.detail?.auto){
    toast('Đã tạo kho đồng bộ mới và sẵn sàng chia sẻ với thiết bị khác.','success');
  }
});
...
function setupSyncManager(){
  ...
  const provider=(config.provider||'jsonstorage');
  syncForm.elements.enabled.checked=!!config.enabled;
  if(syncProviderField){
    syncProviderField.value=provider;
  }
  if(syncForm.elements.endpoint){
    syncForm.elements.endpoint.value=config.endpoint||'';
  }
  if(syncForm.elements.method){
    syncForm.elements.method.value=config.method||'PUT';
  }
  if(syncForm.elements.authScheme){
    syncForm.elements.authScheme.value=config.authScheme||'Bearer';
  }
  if(syncForm.elements.apiKey){
    syncForm.elements.apiKey.value=config.apiKey||'';
  }
  if(syncForm.elements.headers){
    syncForm.elements.headers.value=headersToTextarea(config.headers);
  }
  if(syncForm.elements.pollInterval){
    syncForm.elements.pollInterval.value=Math.max(5, Math.round((config.pollInterval||15000)/1000));
  }
  if(syncForm.elements.githubOwner){
    syncForm.elements.githubOwner.value=config.githubOwner||'';
  }
  if(syncForm.elements.githubRepo){
    syncForm.elements.githubRepo.value=config.githubRepo||'';
  }
  if(syncForm.elements.githubBranch){
    syncForm.elements.githubBranch.value=config.githubBranch||'';
  }
  if(syncForm.elements.githubPath){
    syncForm.elements.githubPath.value=config.githubPath||'';
  }
  setGithubTokenField(config.githubToken||'');
  ...
  if(syncProviderField){
    syncProviderField.addEventListener('change',()=>{
      toggleProviderFields(syncProviderField.value);
      toggleSyncInputs(syncForm.elements.enabled.checked);
    });
  }
...
function getSelectedProvider(){
  if(!syncForm) return 'jsonstorage';
  return (syncProviderField?.value||'jsonstorage').toLowerCase();
}

function setGithubTokenField(token){
  const input=syncForm?.elements?.githubToken;
  if(!input) return;
  input.value='';
  input.dataset.currentToken=token||'';
  input.placeholder=token?'Đã lưu (để trống để giữ nguyên)': 'ghp_...';
}

function toggleProviderFields(provider){
  if(syncJsonFields){
    syncJsonFields.classList.toggle('hidden', provider!=='jsonstorage');
  }
  if(syncGithubFields){
    syncGithubFields.classList.toggle('hidden', provider!=='github');
  }
}

function toggleProviderAvailability(provider,enabled){
  if(syncJsonFields){
    syncJsonFields.querySelectorAll('[data-sync-field]').forEach(field=>{
      if(field.name==='enabled' || field.name==='provider') return;
      const shouldDisable=!enabled || provider!=='jsonstorage';
      field.disabled=shouldDisable;
      field.classList.toggle('opacity-60',shouldDisable);
    });
  }
  if(syncGithubFields){
    syncGithubFields.querySelectorAll('[data-sync-field]').forEach(field=>{
      if(field.name==='enabled' || field.name==='provider') return;
      const shouldDisable=!enabled || provider!=='github';
      field.disabled=shouldDisable;
      field.classList.toggle('opacity-60',shouldDisable);
    });
  }
...
function toggleSyncInputs(enabled){
  ...
  syncForm.querySelectorAll('[data-sync-field]').forEach(field=>{
    ...
    if(field.closest('#sync-jsonstorage-fields') || field.closest('#sync-github-fields')){
      return;
    }
    field.disabled=!enabled;
    field.classList.toggle('opacity-60',!enabled);
  });
  toggleProviderAvailability(provider,enabled);
}
...
function collectSyncConfig({ requireEnabled=false }={}){
  ...
  const pollSeconds=parseInt(syncForm.elements.pollInterval?.value??'15',10);
  const config={
    enabled,
    provider,
  ...
  if(provider==='github'){
    const owner=(syncForm.elements.githubOwner?.value||'').trim();
    const repo=(syncForm.elements.githubRepo?.value||'').trim();
    const branch=(syncForm.elements.githubBranch?.value||'').trim()||'main';
    const path=(syncForm.elements.githubPath?.value||'').trim()||'data/klc-database.json';
    const tokenInput=syncForm.elements.githubToken;
    let token=tokenInput?tokenInput.value.trim():'';
    if(!token && tokenInput?.dataset?.currentToken){
      token=tokenInput.dataset.currentToken;
    }
    if(enabled && (!owner || !repo || !branch || !path)){
      toast('Vui lòng nhập đầy đủ Owner, Repository, Branch và đường dẫn tệp khi bật đồng bộ GitHub.','error');
      return null;
    }
    if(enabled && !token){
      toast('Vui lòng nhập GitHub Personal Access Token để ghi dữ liệu.','error');
      return null;
    }
    config.githubOwner=owner;
    config.githubRepo=repo;
    config.githubBranch=branch;
    config.githubPath=path;
    config.githubToken=token;
    config.endpoint=buildGithubRawUrl({ owner, repo, branch, path });
  }else{
    ...
    config.method=syncForm.elements.method?.value||'PUT';
    config.authScheme=syncForm.elements.authScheme?.value||'Bearer';
    config.apiKey=(syncForm.elements.apiKey?.value||'').trim();
    config.headers=parseHeaderText(syncForm.elements.headers?.value||'');
  }
  return config;
}
...
async function handleSyncSave(evt){
  ...
  refreshSyncEndpointField(saved.endpoint);
  setGithubTokenField(saved.githubToken||'');
