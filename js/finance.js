...
applyPageModules('finance');
watchPageModules('finance');
...
function setupEvents(){
  form.addEventListener('submit', evt => {
    ...
    setTimeout(() => {
      ...
      // subscribeCollection sẽ tự cập nhật bảng & báo cáo
      form.reset();
      hideLoading();
...
function renderTable(data){
  ...
  tableBody.innerHTML = data.map(item => `<tr class="border-b last:border-b-0">
      ...
      <td class="px-3 py-2">${item.type==='income'
        ? '<span class="badge badge-success">Thu</span>'
        : '<span class="badge badge-danger">Chi</span>'}</td>
      ...
      <td class="px-3 py-2 text-right">
        <div class="flex flex-wrap items-center justify-end gap-2">
          ...
          <button class="text-rose-600" data-action="delete" data-id="${item.id}">
            ${user.role==='admin' ? 'Xóa' : 'Xóa (gửi duyệt)'}
          </button>
        </div>
      </td>
    </tr>`).join('');
...
function renderSummary(){
  ...
  const totalIncome = filtered.filter(i => i.type==='income')
    .reduce((s,i)=>s+Number(i.amount||0),0);
  const totalExpense = filtered.filter(i => i.type==='expense')
    .reduce((s,i)=>s+Number(i.amount||0),0);
...
async function handleDelete(id){
  ...
  if (user.role === 'admin'){
    ...
    setTimeout(() => {
      ...
      // subscribeCollection sẽ tự sync; vẫn refresh UI tức thời:
      renderTable(records.filter(r => r.id !== id));
      renderSummary();
      hideLoading();
