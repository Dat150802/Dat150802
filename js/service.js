...
function setupEvents(){
  ...
  warrantyForm.addEventListener('submit',evt=>{
    evt.preventDefault();
    ...
    showLoading('Đang lưu phiếu bảo hành…');
    setTimeout(()=>{
      ...
      warrantyForm.reset();
      hideLoading();
      toast('Đã lưu thông tin bảo hành.','success');
    },400);
  });

  maintenanceForm.addEventListener('submit',evt=>{
    evt.preventDefault();
    if(!ensurePermission(user,'write')) return;
    ...
    showLoading('Đang lưu phiếu bảo dưỡng…');
    setTimeout(()=>{
      appendItem('services',payload);
      services=readCollection('services');
      applyFilter();
      ...
      hideLoading();
      toast('Đã lưu thông tin bảo dưỡng.','success');
    },400);
  });
