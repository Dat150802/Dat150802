...
subscribeCollection('checklists', data => {
  checklists = data;
  applySearchFilter();
});
...
function renderTable(data) {
  ...
  tableBody.innerHTML = data
    .map(
      item => `<tr class="border-b last:border-b-0">
  ...
  tableBody
    ...
    .forEach(btn => btn.addEventListener('click', () => handleDelete(btn.dataset.id)));
}
...
function showDetail(id) {
  ...
  modal.querySelector('[data-field="tasks"]').innerHTML = `<ul class="list-disc pl-5 space-y-1">${record.tasks
    .map(task => `<li><b>${task.time}:</b> ${task.job || '-'}</li>`)
    .join('')}</ul>`;
    ...
    reportBody.innerHTML = sorted
      ...
      <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        ...
        ${item.resultStatus === 'pending'
          ? `<div class="text-xs text-rose-600 mt-2">Lý do: ${item.pendingReason || '-'}</div><div class="text-xs text-slate-500">Gia hạn: ${item.schedule || '-'}</div>`
          : ''}
...
function populateStaffOptions() {
  ...
  if (current && staffList.includes(current)) {
...
function updateSlots() {
  ...
  slotContainer.innerHTML = slots
    .map(
      time => `<div class="flex items-center gap-3">
