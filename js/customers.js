...
function collectFormData(){
  ...
  const purchased = purchasedCheckbox.checked;
  const consulted = Array.from(consultedContainer.querySelectorAll('input'))
...
function renderCustomerDetail(record){
  ...
  purchasedArea.innerHTML = record.purchased
    ? `<div>Mẫu ghế: <b>${record.purchasedModel || '-'}</b></div><div>Giá tiền: <b>${record.purchasedPrice || '-'}</b></div>`
    : '-';

  detailModal.querySelector('[data-field="consulted"]').innerHTML =
    record.consultedList?.length
      ? `<ul class="list-disc pl-5">${record.consultedList.map(item=>`<li>${item}</li>`).join('')}</ul>`
      : '-';
...
function getHistoryBadge(entry){
  ...
  if (entry.type === 'status' && entry.status){
    const map = { appointment:'badge-info', decline:'badge-danger', elsewhere:'badge-warning' };
    const label = { appointment:'Hẹn khách', decline:'Từ chối', elsewhere:'Mua nơi khác' };
...
function handleQuickCareSave(){
  ...
  const input = document.getElementById('detail-care-note');
...
function updateSourceDetailVisibility(){
  ...
  if (detailInput){
    detailInput.placeholder = placeholderMap[sourceSelect.value] || 'Chi tiết nguồn';
  }
}
...
function addConsultedField(){
  ...
  wrapper.innerHTML = `
    <input class="input-brand flex-1" placeholder="Mẫu ghế đã tư vấn">
